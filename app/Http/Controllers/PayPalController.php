...
if($_POST['mc_gross']==5) //Check item name if Intellippt
{    
        $authid = @$_POST['custom'];
        session(['buynotificationmessage' => 'Thanks for your subscription ...!']);
        $checking_trail = Order::where('auth_id',$authid)->get();
        $trail_status[] = "";
    ...
    elseif($_POST['mc_gross'] != 5) //Check item name if Intellippt_keypoints
    {
        $authid = @$_POST['custom'];
        session(['buynotificationmessage' => 'Thanks for your subscription ...!']);
        ...
        if(in_array("no",$trail_status)){
            ...
            $trails = date('Y-m-d H:i:s');
            $buy_account_date = date('Y-m-d H:i:s', strtotime($trails. ' + 7 days'));
            $trial_account_status = "inprogress";
            $buy_account_status = "inprogress";
 ...
 } //End payment completed if
}



    public function paypalrefund(Request $request,$id)
    {
        ...
        if ($token) {
            ...
            if(@$res->state == "completed"){
                ...
                return redirect('/account')->with('buynotificationmessage', 'Your premium pack cancellation has been initiated...!');
            }else{
                return redirect('/account')->with('buynotificationmessage', @$res->message);
            }
